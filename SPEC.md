...
| 代號 | 功能 | 說明 |
|------|------|------|
| F1 | 生成 | 每次隨機產生第 1 或第 2 階物件，並預告下一個 |
| F2 | 投放 | 左右移動後放開，物件從頂部落下；有冷卻時間 |
| F3 | 物理 | 重力、碰撞、堆疊，由 Matter.js 模擬 |
| F4 | 合成 | 同階物件接觸 → 合成高一階；可以連鎖合成；月亮是最高階 |
...
| `SPAWN_WEIGHTS` | `{ 1: 0.6, 2: 0.4 }` | 生成機率 |
...
- 生成器每次只產生 tier 1 或 2，機率依照 `SPAWN_WEIGHTS`。
...
|------|---------|------|
| `drop` | `{ tier, x, y }` | 音效 |
| `merge` | `{ fromTier, toTier, x, y, gained, score }` | 粒子、Pop、閃光圈、飄字、音效 |
| `moon` | `{ x, y }` | 月亮慶祝特效 |
...
- `index.html` 加上 `og:title`、`og:description`、`og:image`（像素月亮圖 `src/og.png`）、`twitter:card`。
...
| AC2 | 生成只會出現第 1 或第 2 階；相同 seed 產生相同序列；有下一個預覽 | 自動測試 ＋ 手動 |
| AC3 | 物件受重力落下，停在地板或其他物件上，不會穿透地板或牆壁 | 自動測試 |
| AC4 | 兩個同階物件接觸後，在同一個 substep 內合成一個高一階物件，位置在兩者中點（經 clamp） | 自動測試 |
