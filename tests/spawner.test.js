...
test('生成 1000 次只出現第 1、2 階，兩者都有，比例接近 60/40', () => {
  const spawner = createSpawner({ rng: createRng(42) });
  const counts = { 1: 0, 2: 0 };
  for (let i = 0; i < 1000; i++) {
    const t = spawner.next();
    assert.ok(t === 1 || t === 2, `出現第 ${t} 階`);
    counts[t]++;
  }
  assert.ok(counts[1] > 0 && counts[2] > 0);
  assert.ok(counts[1] / 1000 > 0.5 && counts[1] / 1000 < 0.7, `第 1 階比例 ${counts[1] / 1000}`);
});
...
test('game 的 current / next 只會是 1 或 2，且同 seed 的兩局序列相同', () => {
  const g1 = newGame({ seed: 5 });
  const g2 = newGame({ seed: 5 });
  for (let i = 0; i < 20; i++) {
    assert.ok([1, 2].includes(g1.current) && [1, 2].includes(g1.next));
