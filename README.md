...
- 每次隨機掉下第 1～4 階的物件，**兩個相同的物件碰在一起就合成高一階**，合成時會帶著慣性往上彈一下，並把周圍推開。
- 合成階級（共 8 次合成才到月亮）：
...
- 人負責：訂需求與遊戲規則、Review Spec / 測試 / diff、試玩並提出調整（例如改成生成 1～4 階、拿掉下一個預覽、加入合成彈跳）、決定何時 commit 與部署。
